...
public class Input{
	...
	public static final int NONE = 0,PRESSED = 1,RELEASED = 2,REPEATED = 3,UP = 2,DOWN = 1,
			NBRE_KEY = 0x15D,NBRE_BUTTON = 10,
			MOUSE_OFFSET = NBRE_KEY + 1,MOUSE_WHEEL_OFFSET = MOUSE_OFFSET + NBRE_BUTTON + 1;
	...
	public static void init(){
		glfwSetScrollCallback(GameEngine.getFrame().getWindowID(), scroll = new GLFWScrollCallback() {
			public void invoke(long window, double xoffset, double yoffset) {
				scroll(window, xoffset, yoffset);
			}
		});
		glfwSetCursorPosCallback(GameEngine.getFrame().getWindowID(), mousePos = new GLFWCursorPosCallback() {
	...
	public static void update(){
		for(Entry<Integer, Integer> set : state.entrySet()){
			...
			int st = set.getValue();
			if(i > -1 && i < NBRE_KEY){
				if(glfwGetKey(GameEngine.getFrame().getWindowID(), i) == 0 && st == NONE)continue;
				if(glfwGetKey(GameEngine.getFrame().getWindowID(), i) == 1 && st == NONE){
					state.replace(i, PRESSED);
				}else if(glfwGetKey(GameEngine.getFrame().getWindowID(), i) == 1 && st == PRESSED){
					state.replace(i, REPEATED);
				}else if(glfwGetKey(GameEngine.getFrame().getWindowID(), i) == 0 && (st == PRESSED || st == REPEATED)){
					state.replace(i, RELEASED);
				}else if(glfwGetKey(GameEngine.getFrame().getWindowID(), i) == 0 && st == RELEASED){
					state.replace(i, NONE);
				}
			}else if(i >= MOUSE_OFFSET && i < MOUSE_OFFSET + NBRE_BUTTON){
				if(glfwGetMouseButton(GameEngine.getFrame().getWindowID(), i - MOUSE_OFFSET) == 0 && st == NONE)continue;
				if(glfwGetMouseButton(GameEngine.getFrame().getWindowID(), i - MOUSE_OFFSET) == 1 && st == NONE){
					state.replace(i, PRESSED);
				}else if(glfwGetMouseButton(GameEngine.getFrame().getWindowID(), i - MOUSE_OFFSET) == 1 && st == PRESSED){
					state.replace(i, REPEATED);
				}else if(glfwGetMouseButton(GameEngine.getFrame().getWindowID(), i - MOUSE_OFFSET) == 0 && (st == PRESSED || st == REPEATED)){
					state.replace(i, RELEASED);
				}else if(glfwGetMouseButton(GameEngine.getFrame().getWindowID(), i - MOUSE_OFFSET) == 0 && st == RELEASED){
					state.replace(i, NONE);
				}
			}
		}
